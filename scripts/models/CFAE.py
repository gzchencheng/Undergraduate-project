...
class CFAE():
    def __init__(self, text_size: object, img_size: object,feature_size=256) -> object:
        ...
        self.text_model = DAE.DAE(input_size=text_size,output_size1=text_size,output_size2=img_size,feature_size=feature_size)
        self.img_model = DAE.DAE(input_size=img_size,output_size1=text_size,output_size2=img_size,feature_size=feature_size)
    ...
    def get_similarity(self,text,img):
        loss_func = nn.MSELoss().cuda()
        text_encode,text_decode,text_decode1 = self.text_model(text)
        img_encode,img_decode,img_decode1 = self.img_model(img)
        return -loss_func(img_encode,text_encode).detach().cpu().numpy()#由于loss越高数据越不相关，因此使用loss的负数作为相关性
    # 获取两个不同模态数据在公共表示空间内的相似性
    def get_samemodal_similarity(self,a,b,type):
        loss_func = nn.MSELoss().cuda()
        if type == 'text':
            a_encode, a_decode, a_decode1 = self.text_model(a)
            b_encode, b_decode, b_decode1 = self.text_model(b)
        else:
            a_encode, a_decode, a_decode1 = self.img_model(a)
            b_encode, b_decode, b_decode1 = self.img_model(b)
    ...
    def search_top3(self,mode,search_data,beta=0.5,img=None,text=None):
        ...
        for i in range(len(search_data)):
            ...
            if mode == 1:
                ...
                similarity = beta * self.get_similarity(text=data,img=img) + (1-beta) * self.get_samemodal_similarity(data,text,type='text')
            else:
                similarity = beta * self.get_similarity(img=data,text=text) + (1-beta) * self.get_samemodal_similarity(data,img,type='img')
            if len(min_heap) < 3:
